...
import { buscarUsuarioTool } from './tools/buscar-usuario.tool';
import { crearUsuarioTool } from './tools/crear-usuario.tool';
import { actualizarUsuarioTool } from './tools/actualizar-usuario.tool';
import { crearReservaTool } from './tools/crear-reserva.tool';
import { buscarReservaTool } from './tools/buscar-reserva.tool';
import { actualizarReservaTool } from './tools/actualizar-reserva.tool';
...
class MCPServer {
  ...
  private registerTools(): void {
    ...
    this.toolRegistry.register(buscarUsuarioTool);
    this.toolRegistry.register(crearUsuarioTool);
    this.toolRegistry.register(actualizarUsuarioTool);
    this.toolRegistry.register(crearReservaTool);
    this.toolRegistry.register(buscarReservaTool);
    this.toolRegistry.register(actualizarReservaTool);
   ...
   */
  private handleJSONRPCError(req: Request, res: Response, error: unknown): void {
    const errorMessage = error instanceof Error ? error.message : String(error);
    this.logger.error(`Error en JSON-RPC: ${errorMessage}`);
  ...
  start(): void {
    this.app.listen(this.port, () => {
      this.logger.info('');
      this.logger.info('🔧 ================================================');
      this.logger.info('🔧 MCP SERVER - GESTIÓN DE RESERVAS');
      this.logger.info('🔧 ================================================');
      ...
      this.logger.info('🔧 ================================================');
      this.logger.info('🔧 Endpoints JSON-RPC (MCP Protocol):');
      this.logger.info('🔧   POST /mcp/tools/list  - Listar herramientas');
      this.logger.info('🔧   POST /mcp/tools/call  - Ejecutar herramienta');
      this.logger.info('🔧 ================================================');
      this.logger.info('🔧 Endpoints REST (debug):');
      this.logger.info('🔧   GET  /health          - Estado del servidor');
      this.logger.info('🔧   GET  /tools           - Listar herramientas');
      this.logger.info('🔧   POST /tools/:name     - Ejecutar herramienta');
      this.logger.info('🔧 ================================================');
      this.logger.info('🔧 Herramientas disponibles:');
